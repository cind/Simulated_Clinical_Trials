...
age <- Adni_Age(adni_imaging)
adni_imaging$AGE <- age
adni_imaging$AGE <- as.numeric(adni_imaging$AGE)
...
adni_imaging$RID     <- factor(adni_imaging$RID)
harm.demogs <- adnimerge[,c("RID", "VISCODE", "PTGENDER", "COLPROT", "APOE4", "DX", "PTEDUCAT", "M")]
...
imaging_data_set     <- full.data.set[,c("RID", "VISCODE", vol.ims, "PTGENDER.x", "COLPROT", "APOE4", "DX", "PTEDUCAT", "AGE", "M")]
...
total.imaging <- cbind(cov.cols.long, harmed.imaging)
total.imaging <- cbind(imaging_data_set[,c("RID", "VISCODE", "STUDY", "APOE4", "AGE", "PTGENDER")], harmed.imaging)
total.imaging <- cbind(total.imaging, imaging_data_set[,im.cols])
